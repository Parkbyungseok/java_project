...
public class A_변수 {
    ...
    public static void main(String[] args) {
        ...
        // NOTE AA02 Type
        // 원시타입, 참조형타입(사용자 정의 타입)
        // 논리형 : boolean(1)
        // 문자형 : char(2)
        // 숫자형 : byte(1), short(2), int(4), long(8), float(4), double(8)

        // NOTE AA03 변수선언
        // 메모리에 타입의 크기만큼 공간 확보하고 시작 주소를 변수에 연결하는 과정
        // type 변수명; ex) int i;

        // NOTE AA04 변수 명명 규칙 : Lower Camel Case
        //Camel Case: 단어를 연결할 때 각 단어의 첫 글자를 대문자로 표기 ex) book + amount = bookAmount
        //LOWER Camel Case : 첫 글자를 소문자로 시작 ex) book + amount = bookAmount
        //Upper Camel Case : 첫 글자를 대문자로 시작 ex) book + amount = BookAmount
        ...
        // 자바의 표준 전수형 타입은 int, 표준 실수형 타입은 double
        // literal의 타입은 표준 타입을 따라감
        longVal = 10000000000L;
        floatVal = 10.001F;
        doubleVal = 100.011234234;
        ...
        // NOTE AA08 명시적 형변환
        // 형변환을 하면 데이터가 손실될 위험이 있는 경우 자동 형변환을 지원하지 않음
